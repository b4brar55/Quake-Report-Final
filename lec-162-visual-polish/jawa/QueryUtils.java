...
public final class QueryUtils {
	  ...
	  public static ArrayList<Earthquake> extractEarthquakes() {
			...
			try {
				  ...
				  JSONObject baseJasonObject = new JSONObject(SAMPLE_JSON_RESPONSE);

				  JSONArray earthquakeArray = baseJasonObject.getJSONArray("features");

				  for (int i =0; i <= earthquakeArray.length();i++) {

						//avde notes de vich (0) di tha i ayi bcoz 0 jo list fetch kiti a array di

						//,more info lec 149

						JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);

						//pla apa features access kiti hun apa properties access kiti

						//& eh properties upper list vich hai
						...
						//hun apa ek object da new instance create krke ode vich add krdage
